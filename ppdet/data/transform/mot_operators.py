...
__all__ = [
    'YUV_MotHistogram', 'Self_MotHistogram', 'MotHistogram','RGBReverse', 'LetterBoxResize', 'MOTRandomAffine', 'Gt2JDETargetThres',
    'Gt2JDETargetMax', 'Gt2FairMOTTarget'
]

@register_op
class YUV_MotHistogram(BaseOperator):
    """
    YUV_MotHistogram
    """

    def __init__(self):
        super(YUV_MotHistogram, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = cv2.cvtColor(channels, cv2.COLOR_YCrCb2BGR)
        return sample



@register_op
class Self_MotHistogram(BaseOperator):
    """
    Self_MotHistogram
    """

    def __init__(self, size_w, size_h):
        super(Self_MotHistogram, self).__init__()
        self.size_w = size_w
        self.size_h = size_h
    ...
    def apply(self, sample, context=None):
        im = sample['image']
        (b, g, r) = cv2.split(im)
        clahe = cv2.createCLAHE(clipLimit=0.3, tileGridSize=(self.size_w, self.size_h)) 
        bH = clahe.apply(b)
        gH = clahe.apply(g)
        rH = clahe.apply(r)
        sample['image'] = cv2.merge((bH, gH, rH))
        return sample

@register_op
class MotHistogram(BaseOperator):
    """
    Histogram
    """

    def __init__(self):
        super(MotHistogram, self).__init__()
